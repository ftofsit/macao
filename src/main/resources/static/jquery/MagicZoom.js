...
function MagicZoom_concat()
{
    ...
    for(var i=0;i<arguments.length;i++)
        for(var j=0;j<arguments[i].length;j++)
            result.push(arguments[i][j]);return result
};
...
function MagicZoom_withoutFirst(sequence,skip)
{
    ...
    for(var i=skip;i<sequence.length;i++)
        result.push(sequence[i]);return result
};
...
function MagicZoom(smallImageContId,smallImageId,bigImageContId,bigImageId,settings)
{
    ...
    if(this.settings["loadingImg"]!='')
    {
        ...
        this.loadingCont.innerHTML=this.settings["loadingText"]
            +'<br/><img border="0" alt="'
            +this.settings["loadingText"]+'" src="'
            +this.settings[" mce_src="'
    +this.settings["loadingImg"]+'"/>';
...
MagicZoom.prototype.hiderect=function()
{
    ...
    if(this.pup){this.pup.style.visibility="hidden"}
...
MagicZoom.prototype.replaceZoom=function(e,ael)
{
    ...
    this.safariOnLoadStarted=false;this.initZoom()
};
...
function MagicZoom_findSelectors(id,zoom)
{
    ...
    for(var i=0;i<aels.length;i++)
    {
        ...
        if(aels[i].rel==id)
        {
            ...
            MagicZoom_addEventListener(aels[i],zoom.settings['thumb_change']
                ,MagicZoom_createMethodReference(zoom,"replaceZoom",aels[i]));

            aels[i].style.outline='0';
            aels[i].mzextend=MagicZoom_extendElement;

            aels[i].mzextend({zoom:zoom,selectThisZoom:function()
                {
            ...
            img.src=aels[i].href;img.style.position='absolute';
...
matches=re.exec(aels[i].rel);bigCont.style.height='300px';
...
var settings={bigImage_always_visible:bigImage_always_visible
    ,drag_mode:drag_mode,header:aels[i].title,opacity:opacity
